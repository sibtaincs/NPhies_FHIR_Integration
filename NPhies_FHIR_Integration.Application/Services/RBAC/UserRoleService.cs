...
var role = await _roleRepository.GetByIdAsync(primaryRole.RoleId);
...
pr.UpdatedAt = DateTime.UtcNow;
       await _userRoleRepository.UpdateAsync(pr);
        }
 }
  ...
  // Set new primary role
    ...
    await _userRoleRepository.UpdateAsync(newPrimary);
 ...
 var hasPermission = permissions.Contains(permissionRecord.Id);
   ...
   var existingRoles = userRoles
            ...
            foreach (var er in existingRoles)
    {
     er.RemovedAt = DateTime.UtcNow;
           await _userRoleRepository.UpdateAsync(er);
            }
