    ...
    if (pageNumber < 1 || pageSize < 1 || pageSize > 100)
{
      ...
      return BadRequest(new { message = "Invalid pagination parameters" });
       ...
       var allRequests = await _taskRequestRepository.GetAllAsync();
  ...
  if (string.IsNullOrWhiteSpace(id))
      {
       ...
       var request = await _taskRequestRepository.GetByIdAsync(id);
          if (request == null)
            {
 ...
 var validStatuses = new[] { "requested", "in-progress", "completed", "failed", "cancelled" };
       ...
       var requests = await _taskRequestRepository.FindAsync(t => t.Status == status && t.Code == "cancel");
            var requestDtos = _mapper.Map<List<TaskRequestDto>>(requests);
...
/// <returns>List of cancellation requests with specified reason</returns>
  ...
  var requests = await _taskRequestRepository.FindAsync(t => t.ReasonCode == reasonCode && t.Code == "cancel");
            var requestDtos = _mapper.Map<List<TaskRequestDto>>(requests);
   ...
   catch (Exception ex)
{
 ...
 if (string.IsNullOrWhiteSpace(ownerId))
  ...
  var requests = await _taskRequestRepository.FindAsync(t => t.OwnerId == ownerId && t.Code == "cancel");
var requestDtos = _mapper.Map<List<TaskRequestDto>>(requests);
   ...
   if (string.IsNullOrWhiteSpace(createDto.TaskId))
   {
    return BadRequest(new { message = "Task ID is required" });
     ...
     // Ensure code is set to "cancel"
       ...
       var request = _mapper.Map<TaskRequest>(createDto);
            await _taskRequestRepository.AddAsync(request);
...
var requestDto = _mapper.Map<TaskRequestDto>(request);
 ...
 _mapper.Map(updateDto, request);
   _taskRequestRepository.Update(request);
            await _taskRequestRepository.SaveChangesAsync();
...
return BadRequest(new { message = "Cancellation Request ID is required" });
            ...
            if (request == null)
   {
     ...
     return NotFound(new { message = "Cancellation request not found" });
            ...
            _taskRequestRepository.Delete(request);
        await _taskRequestRepository.SaveChangesAsync();
